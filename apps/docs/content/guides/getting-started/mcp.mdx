...
To add Supabase to Claude Code, use the following command-line instructions. Replace `<connection-string>` with your connection string.

- **Direct Method**: Pass the full connection string as an argument:

   ```shell
   claude mcp add supabase -- npx -y @modelcontextprotocol/server-postgres "<connection-string>"
   ```


After running either command, the console will respond with:

   ```shell
   Added stdio MCP server supabase with command: npx -y @modelcontextprotocol/server-postgres "<connection-string>"
   ```

When you run Claude Code again (e.g., by restarting), you’ll see the following output indicating the connection:
...
To have Claude use the Supabase MCP server for database queries, include the `supabase:query` tool in your prompt.
